...
- **Single-household editor saves must not materialize Person 2.** The Setup Panel limits Person 2 payload fields to couple households, and write endpoints independently skip Person 2 when `[scenario].household_type = "single"` (or when no `[person2]` table exists under inferred household type). Existing Person 2 data is not deleted implicitly when a household is switched to single.
...
- **A `Retire` event's `year` is the source of truth for retirement timing.** `person.retirement_year` is a legacy v1 field only — `_person_income_components()` reads the person's enabled `Retire` event first, falling back to `person.retirement_year` if no event exists. There is no runtime synthesis step; the person-settings synthesis path (and its Social Security counterpart) was removed as dead code on 2026-08-08 after the v2.0 event-driven timeline made it unreachable.
- **A `SocialSecurity` event's `year` is the source of truth for claiming age; the benefit amount is never stored.** The event carries `year` (computed from an entered claiming age at Setup Panel save time). `monthly_benefit` is *not* written into the event's TOML — `resolve_social_security_monthly_benefit()` looks it up fresh on every read from the person's `social_security_benefits` table (keyed by claiming age, itself preferring an explicit `ss_start_age` or event `age` field, else derived as `year - birth_year`), falling back to the legacy `ss_monthly_benefit` scalar only if the table is absent entirely. This raises `ValueError` if no benefit can be resolved — `validate_scenario()` surfaces that as a scenario error rather than letting it fail silently at projection time.
...
- **Dev-branch workflow (supersedes the 2026-07-05 trunk-based decision):** A long-lived `dev` branch serves as the pre-merge testing buffer. All work happens on `dev`; pushes there do NOT trigger CI or the live deploy (GitHub Actions runs only on `push: branches: [main]`). When ready, merge `dev` → `main` and push with tags — only `main` pushes trigger the test gate and demo/guide deploy. Tags are created only on `main` after merge; `dev` is rebased (or deleted/recreated) from `main` after each merge. The user controls the `main` boundary — never merge or push to `main` without explicit permission. Adopted 2026-08-06 (first dev-only commits: v2.0 event-driven timeline work). The original trunk-based rationale remains in `docs/references/project-governance-and-licensing.md`, which still documents the community-PR path.
