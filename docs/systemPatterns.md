...
- **Historical mode reads annual-return windows from CSV, not hardcoded datasets.** The current contract expects a user/local file path in `simulation.historical_returns_path` with `year` and `return` columns; rolling windows across that dataset define the historical run set.
