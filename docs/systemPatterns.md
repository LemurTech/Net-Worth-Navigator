...
- **Yearly tax calculation should flow through typed contracts.** Federal/state tax-system resolution and annual tax computation now use explicit dataclasses for inputs and outputs, which makes the modeled taxable-income path easier to audit and safer to extend.
