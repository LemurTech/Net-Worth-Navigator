...
- **`scenario_shell.py` has THREE separate HTML-builder functions that render three different deployed pages** (`build_scenario_shell()` → projection.html, `build_compare_page()` → compare.html, and the definitions page lives in a separate module). Copy-pasting boot/init JS between them without checking whether the target elements actually exist on that page is a recurring source of silent breakage — an unguarded `getElementById(...).addEventListener(...)` on a null element throws and aborts every later statement in the same `DOMContentLoaded` handler, with zero visible error to the user (just an empty page). This exact bug (help-mode toggle wiring copy-pasted into `build_compare_page()`, which has no help-mode UI) sat undetected for 2 days because nothing reloaded the compare page after the copy-paste. **Whenever touching one of these builder functions, reload and console-check the OTHER pages too** — they can silently break from unrelated-looking edits.
