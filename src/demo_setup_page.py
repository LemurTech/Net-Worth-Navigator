...
def _person_income(person: dict, title: str) -> str:
    ...
    blocks.append(_section("Retirement Contributions", _fields(contribution_items)))
    ...
    blocks.append(_section("Employer Match, Routing & Ownership", _fields(match_items)))
...
.scenario-select { min-width:185px; cursor:pointer; } .readonly { margin:14px 0; padding:10px 13px; border:1px solid rgba(125,211,252,.35); background:rgba(14,165,233,.08); border-radius:8px; color:var(--muted); font-size:13px; }
...
.event-card { border:1px solid var(--border); border-radius:9px; padding:13px; margin-bottom:10px; background:var(--panel2); } .event-head { display:flex; justify-content:space-between; gap:12px; align-items:center; margin-bottom:11px; } .event-head strong { display:block; font-size:14px; } .event-head span { color:var(--muted); font-size:12px; } .badge { border:1px solid rgba(74,222,128,.45); color:#86efac; background:rgba(34,197,94,.1); border-radius:999px; padding:3px 8px; font-size:11px; white-space:nowrap; } .badge.disabled { color:#cbd5e1; border-color:#475569; background:#1e293b; }
...
def build_demo_setup_page(
    *, config_path: Path, output_path: Path, slug: str,
    ...
    for key, person in people:
        ...
        ])) + '<div class="table-wrap"><table><thead><tr><th>Claiming Age</th><th>Monthly Benefit</th></tr></thead><tbody>' + "".join(
    ...
    accounts = _section("Data Source", _fields([("Mode", _text((config.get("data_source") or {}).get("mode")), "")]))
    accounts += _section("Manual Starting Balances", _fields(balance_items))
    accounts += _section("Property Values", _fields(property_items or [("Properties", "None configured", "")]))
    accounts += _section("Liabilities", '<div class="table-wrap"><table><thead><tr><th>Name</th><th>Type</th><th>Annual Rate</th><th>Monthly Payment</th><th>Escrow</th><th>Extra</th><th>Starting Balance</th></tr></thead><tbody>' + liability_rows + "</tbody></table></div>")
    ...
    events_html = "".join(_event_card(event) for event in events) or '<div class="section">No events configured.</div>'
